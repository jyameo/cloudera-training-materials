...
callCmApi () {
	...
	echo "--------------------"
	statusURL="http://lion:7180/api/v9/commands/$id"
...
checkPending () {
	...
	checkCommands=$(curl -X GET -H "Content-Type:application/json" -u admin:admin http://lion:7180/api/v9/clusters/cluster/services/$1/commands|grep -C4 items|grep active|cut -d " " -f7- | sed 's/[,:"]//g')
	...
	echo
	curl -X GET -H "Content-Type:application/json" -u admin:admin http://lion:7180/api/v9/clusters/cluster/services/$1/commands
}
...
backupDeployment () {
	...
	'http://lion:7180/api/v9/cm/deployment' > $2/catchup/working/$1
}
##############################################################
...
generateJson () {
	...
	curl -X GET -H "Content-Type:application/json" -u admin:admin 'http://lion:7180/api/v9/hosts'|grep 'hostId\|ipAddress\|hostname' > /home/training/training_materials/admin/scripts/catchup/working/hostIds.txt
	...
	echo "Copying resulting JSON to lion."
	echo
	ssh training@lion mkdir -p $SCRIPTS_DIR/catchup/working
	scp $OUTPUT_FILE training@lion:$SCRIPTS_DIR/catchup/working/$1	
} 
...
putJson () {
	...
	'http://lion:7180/api/v9/cm/deployment?deleteCurrentDeployment=true' \
...
activateParcels () {
	...
	echo "***************************"
	echo
	echo "Copying parcel to parcel-cache on elephant"
	sudo cp /opt/cloudera/parcel-repo/$PARCEL_NAME \
	/opt/cloudera/parcel-cache/$PARCEL_NAME 2>&1 | sed -e 's/^/elephant output> /' &
	echo
	echo "Copying parcel to parcel-cache on tiger"
	ssh training@tiger -o StrictHostKeyChecking=no \
	sudo cp /opt/cloudera/parcel-repo/$PARCEL_NAME \
	/opt/cloudera/parcel-cache/$PARCEL_NAME 2>&1 | sed -e 's/^/tiger output> /' &
	echo
	echo "Copying parcel to parcel-cache on horse"
	ssh training@horse -o StrictHostKeyChecking=no \
	sudo cp /opt/cloudera/parcel-repo/$PARCEL_NAME \
	/opt/cloudera/parcel-cache/$PARCEL_NAME 2>&1 | sed -e 's/^/horse output> /' &
	echo
	echo "Copying parcel to parcel-cache on monkey"
	ssh training@monkey -o StrictHostKeyChecking=no \
	sudo cp /opt/cloudera/parcel-repo/$PARCEL_NAME \
	/opt/cloudera/parcel-cache/$PARCEL_NAME 2>&1 | sed -e 's/^/monkey output> /' &
	echo
	echo "Copying parcel to parcel-cache on lion"
	ssh training@lion -o StrictHostKeyChecking=no \
	sudo cp /opt/cloudera/parcel-repo/$PARCEL_NAME \
	/opt/cloudera/parcel-cache/$PARCEL_NAME 2>&1 | sed -e 's/^/lion output> /' &
	echo 
	...
	while [[ $parcelStatus != "ACTIVATED" ]]
		do
		parcelStatus=$(curl -X GET -H "Content-Type:application/json" -u admin:admin 'http://lion:7180/api/v9/clusters/cluster/parcels/products/CDH/versions/5.9.0-1.cdh5.9.0.p0.23' | grep stage | cut -d " " -f5-|sed 's/[",]//g')
		echo $parcelStatus
		sleep 10
	done
	echo 
...
firstRun () {
	...
	echo $(ssh -o StrictHostKeyChecking=no training@lion date)
	echo "***********************************************************************************"
	
	callCmApi http://lion:7180/api/v9/clusters/cluster/commands/firstRun POST
...
startCMMS () {
	...
	callCmApi http://lion:7180/api/v9/cm/service/commands/start POST
...
firstRunHDFS () {
	...
	callCmApi http://lion:7180/api/v9/clusters/cluster/services/hdfs/commands/firstRun POST
	
	#don't move on until HDFS has started.
	hdfsStat=""
	while [[ $hdfsStat != "STARTED" ]]
		do
		hdfsStat=$(curl -X GET -H "Content-Type:application/json" -u admin:admin 'http://lion:7180/api/v9/clusters/cluster/services/hdfs'|grep STARTED|cut -d " " -f5-|sed 's/[",]//g')
		echo $hdfsStat
		sleep 10
	done
}
...
updateDNPerms () {	
	...
	echo
	checkPending "hdfs"
	callCmApi http://lion:7180/api/v9/clusters/cluster/services/hdfs/roleConfigGroups/hdfs-DATANODE-1/config PUT "-d @$SCRIPTS_DIR/catchup/setDNperms.json"
	checkPending "hdfs"
	callCmApi http://lion:7180/api/v9/clusters/cluster/services/hdfs/roleConfigGroups/hdfs-DATANODE-BASE/config PUT "-d @$SCRIPTS_DIR/catchup/setDNperms.json"
}
...
restartHDFS () {
	...
	echo "***************************"
	callCmApi http://lion:7180/api/v9/clusters/cluster/services/hdfs/commands/restart POST
}
...
configHDFS () {	
	...
	callCmApi http://lion:7180/api/v9/cm/service/roleCommands/restart POST "-d @/home/training/training_materials/admin/scripts/catchup/reportsManager.json"
}
...
deployCC () {
	...
	callCmApi http://lion:7180/api/v9/clusters/cluster/commands/deployClientConfig POST
...
startYARN () {
	...
	callCmApi http://lion:7180/api/v9/clusters/cluster/services/yarn/commands/start POST
...
restartYARN () {
	...
	callCmApi http://lion:7180/api/v9/clusters/cluster/services/yarn/commands/restart POST
...
addSparkService () {
	...
	callCmApi http://lion:7180/api/v9/clusters/cluster/services POST "-d @$SCRIPTS_DIR/catchup/working/addSparkService.json"
	...
	callCmApi http://lion:7180/api/v9/clusters/cluster/services/yarn/roleConfigGroups/yarn-RESOURCEMANAGER-BASE/config PUT "-d @$SCRIPTS_DIR/catchup/yarnspark1.json"
	#'{"items":[{"name":"yarn_scheduler_maximum_allocation_mb","value":"1536"}]}' 
	callCmApi http://lion:7180/api/v9/clusters/cluster/services/yarn/roleConfigGroups/yarn-NODEMANAGER-BASE/config PUT "-d @$SCRIPTS_DIR/catchup/yarnspark2.json"
	#'{"items":[{"name":"yarn_nodemanager_resource_memory_mb","value":"1536"}]}' 
	callCmApi http://lion:7180/api/v9/clusters/cluster/services/yarn/roleConfigGroups/yarn-NODEMANAGER-1/config PUT "-d @$SCRIPTS_DIR/catchup/yarnspark2.json"
	#'{"items":[{"name":"yarn_nodemanager_resource_memory_mb","value":"1536"}]}' 
...
startSpark () {
	...
	echo ""
	callCmApi http://lion:7180/api/v9/clusters/cluster/services/spark_on_yarn/commands/start POST
...
restartSpark () {
	...
	echo "Starting Spark service"
	echo ""
	callCmApi http://lion:7180/api/v9/clusters/cluster/services/spark_on_yarn/commands/restart POST
...
addFlumeService () {
	...
	echo 
	callCmApi http://lion:7180/api/v9/clusters/cluster/services POST "-d @$SCRIPTS_DIR/catchup/working/addFlumeService.json"
}
...
exerciseFlume () {
	...
	echo
	callCmApi http://lion:7180/api/v9/clusters/cluster/services/flume/commands/start POST
	...
	echo
	callCmApi http://lion:7180/api/v9/clusters/cluster/services/flume/commands/stop POST
...
addSqoopService () {
	...
	echo 
	callCmApi http://lion:7180/api/v9/clusters/cluster/services POST "-d @$SCRIPTS_DIR/catchup/working/addSqoopService.json"
	echo "Redeploying the Sqoop gateway config."
	callCmApi http://lion:7180/api/v9/clusters/cluster/services/sqoop_client/commands/deployClientConfig POST "-d @$SCRIPTS_DIR/catchup/deploySqoopCC.json"
}
...
addZKService () {
	...
	echo 
	callCmApi http://lion:7180/api/v9/clusters/cluster/services POST "-d @$SCRIPTS_DIR/catchup/working/addZKService.json"
}
...
firstRunZK () {
	...
	echo ""
	echo
	callCmApi http://lion:7180/api/v9/clusters/cluster/services/zookeeper/commands/firstRun POST
}
...
addHiveService () {
	...
	echo 
	callCmApi http://lion:7180/api/v9/clusters/cluster/services POST "-d @$SCRIPTS_DIR/catchup/working/addHiveService.json"
}
...
enableHiveOnSpark () {	
	...
	echo
	callCmApi http://lion:7180/api/v9/clusters/cluster/services/hive/config PUT "-d @$SCRIPTS_DIR/catchup/hiveonspark.json"
	
	echo
	echo "Set spark executor cores for hive."
	echo 
	#callCmApi http://lion:7180/api/v9/clusters/cluster/services/hive/roleConfigGroups/hive-HIVESERVER2-BASE/config PUT "-d @$SCRIPTS_DIR/catchup/hivesparkex.json"
	callCmApi http://lion:7180/api/v9/clusters/cluster/services/hive/roleConfigGroups/hive-HIVESERVER2-BASE/config PUT "-d @$SCRIPTS_DIR/catchup/hivesparkex.json"
}
...
firstRunHive () {		
	...
	echo ""
	echo
	callCmApi http://lion:7180/api/v9/clusters/cluster/services/hive/commands/firstRun POST
}
#NEW 2016
restartHive () {
	callCmApi http://lion:7180/api/v9/clusters/cluster/services/hive/commands/restart POST
}
...
addImpalaService () {
	...
	echo 
	callCmApi http://lion:7180/api/v9/clusters/cluster/services POST "-d @$SCRIPTS_DIR/catchup/working/addImpalaService.json"
	echo
	echo "Configuring HDFS for Impala."
	echo 
	callCmApi http://lion:7180/api/v9/clusters/cluster/services/hdfs/config PUT "-d @$SCRIPTS_DIR/catchup/hdfsForImpala.json"
	callCmApi http://lion:7180/api/v9/clusters/cluster/services/hdfs/roles/hdfs-DATANODE-1/config PUT "-d @$SCRIPTS_DIR/catchup/hdfsRoleForImpala.json"
	callCmApi http://lion:7180/api/v9/clusters/cluster/services/hdfs/roles/hdfs-DATANODE-BASE/config PUT "-d @$SCRIPTS_DIR/catchup/hdfsRoleForImpala.json"
	echo
	...
	echo 
	callCmApi http://lion:7180/api/v9/clusters/cluster/services/hive/commands/deployClientConfig POST "-d @$SCRIPTS_DIR/catchup/deployHiveCC.json"	
}
...
firstRunImpala () {
	...
	echo ""
	echo
	callCmApi http://lion:7180/api/v9/clusters/cluster/services/impala/commands/firstRun POST
	
}

startImpala () {
	callCmApi http://lion:7180/api/v9/clusters/cluster/services/impala/commands/start POST
}
...
exerciseImpala () {	
	...
	'http://lion:7180/api/v9/clusters/cluster/services/impala/impalaQueries?from=2015-05-03T18%3A07%3A22.189Z&to=2020-05-05T18%3A37%3A22.189Z'	
	echo ""
}
...
addHttpFs () {
	...
	echo 
	callCmApi http://lion:7180/api/v9/clusters/cluster/services/hdfs/roles/ POST "-d @$SCRIPTS_DIR/catchup/working/addHttpFs.json"
}
...
addSparkGateway () {
	...
	echo 
	callCmApi http://lion:7180/api/v9/clusters/cluster/services/spark_on_yarn/roles/ POST "-d @$SCRIPTS_DIR/catchup/working/addSparkGateway.json"
	echo
	echo "Deploying Spark client configurations."
	echo 
	callCmApi http://lion:7180/api/v9/clusters/cluster/services/spark_on_yarn/commands/deployClientConfig POST "-d @$SCRIPTS_DIR/catchup/deploySparkCCe.json"
}
...
addOozieService () {
	...
	echo 
	callCmApi http://lion:7180/api/v9/clusters/cluster/services POST "-d @$SCRIPTS_DIR/catchup/working/addOozieService.json"
}
...
echo
	...
	echo 
	callCmApi http://lion:7180/api/v9/clusters/cluster/services POST "-d @$SCRIPTS_DIR/catchup/working/addHueService.json"
}
...
startHttpFs () {
	...
	echo
	callCmApi http://lion:7180/api/v9/clusters/cluster/services/hdfs/roleCommands/start POST "-d @$SCRIPTS_DIR/catchup/startHttpFs.json"
}
...
firstRunOozie () {	
	...
	echo
	callCmApi http://lion:7180/api/v9/clusters/cluster/services/oozie/commands/firstRun POST
}
...
stopHue () {
	...
	echo 
	callCmApi http://lion:7180/api/v9/clusters/cluster/services/hue/commands/stop POST
	checkPending "hue"
}
...
firstRunHue () {
	...
	echo
	callCmApi http://lion:7180/api/v9/clusters/cluster/services/hue/commands/firstRun POST
}
...
preHAConfigurations () {
	...
	callCmApi http://lion:7180/api/v9/clusters/cluster/services/hue/commands/stop POST
	callCmApi http://lion:7180/api/v9/clusters/cluster/services/oozie/commands/stop POST
	callCmApi http://lion:7180/api/v9/clusters/cluster/services/impala/commands/stop POST
	callCmApi http://lion:7180/api/v9/clusters/cluster/services/hive/commands/stop POST
...
editHDFSHAJSON () {
	...
	curl -X GET -H "Content-Type:application/json" -u admin:admin \
	'http://lion:7180/api/v9/hosts'|grep 'hostId\|ipAddress\|hostname' > /home/training/training_materials/admin/scripts/catchup/working/hostIds.txt
...
configHDFSHA () {
	...
	callCmApi http://lion:7180/api/v9/clusters/cluster/services/hdfs/commands/hdfsEnableNnHa POST "-d @$SCRIPTS_DIR/catchup/working/setHDFSHA.json"
	
	echo
	echo "Waiting for hdfsEnableNnHa and any other pending HDFS commands to complete."
	echo 

	hdfsCommands=$(curl -X GET -H "Content-Type:application/json" -u admin:admin 'http://lion:7180/api/v9/clusters/cluster/services/hdfs/commands'|grep -C4 items|grep active|cut -d " " -f7- | sed 's/[,:"]//g')
	...
	echo "Showing GET results again."
	echo
	curl -X GET -H "Content-Type:application/json" -u admin:admin 'http://lion:7180/api/v9/clusters/cluster/services/hdfs/commands'
	...
	callCmApi http://lion:7180/api/v9/clusters/cluster/services/hive/commands/hiveUpdateMetastoreNamenodes POST
...
testHDFSHA () {
	...
	echo	
	callCmApi http://lion:7180/api/v9/clusters/cluster/services/hdfs/roleCommands/restart POST "-d @$SCRIPTS_DIR/catchup/namenodeE.json"
	
	echo
	echo "Restarting NN on Tiger which should make Elephant the active NN again."
	echo	
	callCmApi http://lion:7180/api/v9/clusters/cluster/services/hdfs/roleCommands/restart POST "-d @$SCRIPTS_DIR/catchup/namenodeT.json"
}
...
exerciseFairSched () {
	...
	callCmApi http://lion:7180/api/v9/clusters/cluster/services/yarn/roleConfigGroups/yarn-NODEMANAGER-BASE/config PUT "-d @$SCRIPTS_DIR/catchup/setPools.json"
	checkPending "yarn"
	...
	callCmApi http://lion:7180/api/v9/clusters/cluster/services/yarn/roleConfigGroups/yarn-NODEMANAGER-1/config PUT "-d @$SCRIPTS_DIR/catchup/setPools.json"
	checkPending "yarn"
	...
	#callCmApi http://lion:7180/api/v9/clusters/cluster/commands/refresh POST
	...
	callCmApi http://lion:7180/api/v9/clusters/cluster/services/yarn/config PUT "-d @$SCRIPTS_DIR/catchup/dynamicYarn.json "
	...
	callCmApi http://lion:7180/api/v9/clusters/cluster/services/impala/config PUT "-d @$SCRIPTS_DIR/catchup/dynamicImpala.json"
	
	#echo
	#echo "***Restarting HDFS, YARN, Spark, ZK to apply changes."
	#echo
	#callCmApi http://lion:7180/api/v9/clusters/cluster/commands/restart POST "-d @$SCRIPTS_DIR/catchup/restartStale.json"
	echo
...
killDataNode () {
	...
	echo
	callCmApi http://lion:7180/api/v9/clusters/cluster/services/hdfs/roleCommands/stop POST "-d @$SCRIPTS_DIR/catchup/datanodeE.json"
}

startDataNode () {
	echo
	echo "Checking if the datanode is stopped"
	status=$(curl -X GET -H "Content-Type:application/json" -u admin:admin http://lion:7180/api/v9/clusters/cluster/services/hdfs/roles/hdfs-DATANODEe | grep roleState | awk '{print $3}'| sed 's/[",]//g')
	echo "DataNode on elephant is "$status
	if [[ $status == "STOPPED" ]]; then
		echo "Starting DataNode on elephant"
		echo
		callCmApi http://lion:7180/api/v9/clusters/cluster/services/hdfs/roleCommands/start POST "-d @$SCRIPTS_DIR/catchup/datanodeE.json"
	fi
}
...
configEmail () {
	...
	callCmApi http://lion:7180/api/v9/cm/service/roleConfigGroups/mgmt-ALERTPUBLISHER-BASE/config PUT "-d @$SCRIPTS_DIR/catchup/setEmail.json"

	echo
	echo "Restarting CM Management Services to apply email config."
	echo
	
	callCmApi http://lion:7180/api/v9/cm/service/commands/restart POST
	...
	echo
	ssh training@lion -o StrictHostKeyChecking=no ~/training_materials/admin/scripts/catchup/mail.exp
	sleep 5
}
...
heapOTrouble () {
	...
	echo
	curl -X GET -v -H "Content-Type:application/json" -u admin:admin \
	'http://lion:7180/api/v9/clusters/cluster/services/yarn/roleConfigGroups/yarn-GATEWAY-BASE/config'
	...
	callCmApi http://lion:7180/api/v9/clusters/cluster/services/yarn/roleConfigGroups/yarn-GATEWAY-BASE/config PUT "-d @$SCRIPTS_DIR/catchup/setHeap.json"
...
# curl -X GET -H "Content-Type:application/json" -u admin:admin \
# 'http://lion:7180/api/v9/cm/deployment' > $SCRIPTS_DIR/catchup/deployment2.json.
#
